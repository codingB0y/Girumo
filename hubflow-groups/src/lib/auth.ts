// Auth leve, sem dependências e compatível com Edge (middleware) E Node (rotas).
// Cookie de sessão = base64url(payload).hmacHex, assinado com AUTH_SECRET.
// NÃO é multi-tenant — é um portão de 1 dono (o lojista) + token p/ a engine.

export const SESSION_COOKIE = "dz_session";

// Defaults p/ rodar sem configurar nada; troque via env em produção.
export const APP_PASSWORD = process.env.APP_PASSWORD ?? "viraimoda";
...
/** Gera um cookie de sessão assinado p/ uma conta (sub = accountId). */
export async function signSession(accountId: string): Promise<string> {
  const payload = b64url(JSON.stringify({ sub: accountId, iat: Date.now() }));
  const sig = await hmacHex(payload);
  return `${payload}.${sig}`;
}

/** Valida o cookie (assinatura + validade) e devolve o accountId, ou null. Edge-safe. */
