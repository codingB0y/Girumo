...
Within the application’s database schema, Users refer to a dedicated table designed to store comprehensive account and profile information. This table serves as the primary repository for user data, linking individual profiles to their respective authentication identifiers and organizational affiliations. By centralizing account details and authentication links, the Users table enables the system to maintain accurate records of each registered individual's identity, permissions, and status, thereby supporting the broader functionality of the application.<SEP>A database table storing user profile information associated with specific tenants.
