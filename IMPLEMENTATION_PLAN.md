...
## ✈️ Pré-voo — o que só o IGOR pode destravar (nada disso é sessão de código)

| # | Ação | Destrava | Urgência |
|---|---|---|---|
| 1 | **Rotacionar a Service Role Key no painel do Supabase** (dev e prod) e atualizar os envs no Vercel/Coolify — pendente desde o Sprint 1 (TASK_PROGRESS item 1) | Segurança de TODOS os tenants | 🔴 hoje, ~5 min |
| 2 | Confirmar planos/preços vigentes (R$197/297/497?) | P0.6 | 🔴 semana 1 |
| 3 | Aprovar textos: templates de automação (P0.7), passos do playbook (P1.8), copys do Kit de Anúncio (P1.8), presets de campanha (P1.12), copys da biblioteca (P1.14) | vários | 🟠 conforme os itens chegam |
| 4 | Prints do Gerenciador Meta pro passo-a-passo do Kit de Anúncio (`public/ads-guide/`) | P1.8 | 🟠 semana 4–5 |
| 5 | Provas visuais do case Mega Stock (vídeo do evento, fotos do galpão) — pendência do PROJECT_CONTEXT | landing/P0.6 | 🟠 |
| 6 | Exportar 10–20 conversas reais de WhatsApp com clientes | P2.19 (VOC) | 🟡 |
| 7 | Decidir: meta também em R$ além de contatos? (recomendo sim) | P0.5 | 🟡 |
...
## [ ] P0.8 🟢 Hardening do middleware: `resolveSecret` fail-safe (previne o outage de 03/jul)
**Contexto:** risco latente documentado no PROJECT_CONTEXT (incidente 2026-07-03): `apps/web/src/lib/runtime-secrets.ts:13` ainda dá `throw` no module-eval do Edge runtime (via `middleware.ts`→`auth.ts`). Se qualquer secret (ENGINE_TOKEN/AUTH_SECRET/CRON_SECRET) sumir num deploy futuro, produção inteira volta a responder 500 MIDDLEWARE_INVOCATION_FAILED — todos os clientes fora do ar. O hardening já foi sugerido no próprio doc: tornar `resolveSecret` fail-safe como o `enforceEnvironmentValidation` já é.
**Arquivos:** `apps/web/src/lib/runtime-secrets.ts` · conferir cadeia `middleware.ts` → `lib/auth.ts` → `lib/env-validator.ts`.
**Fazer:** em produção, secret ausente NÃO derruba o module-eval do Edge: logar erro alto e degradar de forma segura (requests que dependem do secret ausente falham com 401/503 controlado, o resto do site continua). Manter o comportamento estrito em dev (falhar cedo e claro). Seguir o padrão fail-safe que `enforceEnvironmentValidation` já usa.
**Aceite:** simular secret ausente em build/preview → site continua servindo páginas públicas e login; rotas dependentes do secret respondem erro controlado (não 500 global); com todos os secrets presentes, comportamento idêntico ao atual; lint/build ok.
...
```
Antes de tudo: Igor rotaciona a Service Role Key (pré-voo #1 — não é sessão de código)
Semana 1:  P0.1 → P0.8 → P0.2 → P0.3 → P0.4  (produção blindada + prova de valor destravada)
