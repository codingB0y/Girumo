 ...
 * sincronizados do tenant: não existe hoje um jeito de escolher um
 * subconjunto (`groups.selected` não é usado em lugar nenhum da UI atual —
 * não é um sinal confiável de "grupo ativo", por isso não foi reaproveitado
 * aqui).
 *
  ...
  return {
    ...
    async listGroupsByTenant(tenantIds) {
      ...
      const [{ data: groups, error: groupsError }, { data: recentLeads, error: leadsError }] = await Promise.all([
        supabase
          .from("groups")
          .select("id, tenant_id, whatsapp_group_id, members, capacity")
...
export async function runAutomationScansTick(deps: ScanDeps, now: Date = new Date()): Promise<ScansSummary> {
  ...
  const groupsByTenant = await deps.listGroupsByTenant(tenantIds);
  ...
  for (const candidate of groupStalledCandidates(stalledAutomations, groupsByTenant)) {
