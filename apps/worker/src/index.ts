...
async function main(): Promise<void> {
  ...
  // 1) eventos + manutenção, no pollMs de sempre.
  //    Ciclo e manutenção têm try/catch PRÓPRIOS (como no while original): uma
  //    falha em runTick não pode impedir a manutenção de rodar no mesmo tick —
  //    lease vencido, progresso de broadcast e agendamento não dependem do
  //    resto do ciclo ter ido bem.
