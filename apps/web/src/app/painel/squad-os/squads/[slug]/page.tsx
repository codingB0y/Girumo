      ...
      <div className="mx-auto max-w-[1000px] px-4 py-6 sm:px-6">
        <div className="rounded-3xl border border-volt-950/[0.08] bg-white p-8 text-center">
          ...
          <Link
            ...
            className="mt-4 inline-flex items-center gap-2 text-sm text-cobalt-500 hover:text-cobalt-700"
          >
    ...
    <div className="mx-auto max-w-[1000px] space-y-6 px-4 py-6 sm:px-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <nav className="font-data text-[11px] uppercase tracking-wider text-aco/50">
          <Link href="/painel/squad-os" className="hover:text-cobalt-500">Squad OS</Link>
          {" / "}
          <Link href="/painel/squad-os/squads" className="hover:text-cobalt-500">Squads</Link>
          {" / "}
          <span className="text-volt-950">{squad.name}</span>
        </nav>
      </div>
      ...
      <div className="rounded-3xl border border-volt-950/[0.08] bg-white p-6">
        ...
        {/* Meta info */}
        <div className="mt-5 grid grid-cols-2 gap-4 border-t border-volt-950/[0.06] pt-5 sm:grid-cols-4">
          <div>
            <p className="font-data text-[10px] uppercase tracking-wider text-aco/40">Reputação</p>
            <p className="mt-1 text-sm font-semibold text-volt-950">{squad.reputation_score}/100</p>
          </div>
          <div>
            <p className="font-data text-[10px] uppercase tracking-wider text-aco/40">Missions</p>
            <p className="mt-1 text-sm font-semibold text-volt-950">{missions.length}</p>
          </div>
          <div>
            <p className="font-data text-[10px] uppercase tracking-wider text-aco/40">Última entrega</p>
            <p className="mt-1 text-sm text-volt-950">{squad.last_delivery ?? "—"}</p>
          </div>
          <div>
            <p className="font-data text-[10px] uppercase tracking-wider text-aco/40">Próxima ação</p>
            <p className="mt-1 text-sm font-medium text-cobalt-500">{squad.next_action ?? "—"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        {/* Missions */}
        <div className="rounded-3xl border border-volt-950/[0.08] bg-white p-5 lg:col-span-2">
          <h2 className="font-display text-base font-bold text-volt-950">Missions</h2>
          {missions.length === 0 ? (
            ...
            <div className="mt-4 space-y-2">
              {missions.map((m) => {
                ...
                return (
                  <div
                    key={m.id}
                    className="flex items-center gap-3 rounded-xl border border-volt-950/[0.05] px-4 py-3 transition hover:border-cobalt-500/20 hover:bg-canvas-100/30"
                  >
                    <MissionIcon status={m.status} />
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium text-volt-950">{m.title}</p>
        ...
        {/* Agents */}
        <div className="rounded-3xl border border-volt-950/[0.08] bg-white p-5">
          <h2 className="font-display text-base font-bold text-volt-950">Agents</h2>
          <div className="mt-4 space-y-3">
            {agents.slice(0, 4).map((agent) => (
              <div
                key={agent.id}
                className="flex items-center gap-3 rounded-xl px-3 py-2 transition hover:bg-canvas-100/50"
              >
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-cobalt-500/10">
                  <Bot className="h-4 w-4 text-cobalt-500" />
                </div>
                <div className="min-w-0 flex-1">
