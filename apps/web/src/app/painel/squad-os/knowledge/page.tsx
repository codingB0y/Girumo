    ...
    <div className="mx-auto max-w-[1000px] space-y-6 px-4 py-6 sm:px-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          { label: "Projeto", desc: "Arquitetura", count: 8, color: "bg-iris/10 text-iris" },
          ...
          >
            ...
            <p className="font-display mt-2 text-lg font-bold text-breu">{layer.count}</p>
      ...
      <div className="rounded-3xl border border-breu/[0.08] bg-white p-6">
        ...
        <p className="mt-1 text-xs text-aco/55">
          Decisões registradas que guiam o desenvolvimento do HubFlow
        </p>
        ...
        <div className="mt-5 space-y-3">
          {activeDecisions.map((d) => {
            ...
            return (
              <div
                key={d.id}
                className="rounded-xl border border-breu/[0.06] px-5 py-4 transition hover:border-iris/20 hover:bg-bruma/20"
              >
                <div className="flex items-start gap-3">
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-iris/10">
                    <Icon className="h-4 w-4 text-iris" />
                  </span>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-breu">{d.title}</p>
                    ...
                    <div className="mt-2 flex items-center gap-3">
                      <span className="font-data rounded-full bg-bruma px-2 py-0.5 text-[9px] uppercase tracking-wider text-aco/50">
                        {d.category}
                      </span>
      ...
      <div className="rounded-3xl border border-breu/[0.08] bg-white p-6">
        <h2 className="font-display text-base font-bold text-breu">Playbooks</h2>
        <p className="mt-1 text-xs text-aco/55">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            { name: "Brand", desc: "Tons, tipografia, paleta iris/breu" },
            { name: "Produto", desc: "PRD template, priorização, roadmap" },
            { name: "Operações", desc: "Deploy, CI/CD, monitoramento" },
          ].map((pb) => (
            <div
              key={pb.name}
              className="rounded-xl border border-breu/[0.05] px-4 py-3 transition hover:border-iris/20"
            >
              <p className="text-sm font-medium text-breu">{pb.name}</p>
