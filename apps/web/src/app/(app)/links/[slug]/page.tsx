  ...
  return (
    <>
      ...
      <main className="flex-1 space-y-5 p-6">
        <Link href="/links" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-800">
        ...
        <Card className="flex items-center gap-3 p-5">
          ...
          <div>
            ...
            <p className="text-2xl font-semibold text-slate-900">{a.total.toLocaleString("pt-BR")}</p>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          <Card>
            ...
            <CardContent className="space-y-2">
              {a.byDay.length === 0 && <p className="text-sm text-slate-400">Sem cliques ainda.</p>}
              {a.byDay.map((d) => (
                <div key={d.date} className="flex items-center gap-3 text-sm">
                  ...
                  <span className="w-8 text-right text-slate-700">{d.count}</span>
                </div>
              ))}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {a.bySource.length === 0 && <p className="text-sm text-slate-400">Sem cliques ainda.</p>}
              {a.bySource.map((s) => (
                <div key={s.source} className="flex items-center gap-3 text-sm">
                  <span className="w-24 shrink-0 truncate text-slate-500">{s.source}</span>
                  <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100">
                    <div className="h-full rounded-full bg-blue-500" style={{ width: `${(s.count / maxSrc) * 100}%` }} />
                  </div>
                  <span className="w-8 text-right text-slate-700">{s.count}</span>
                </div>
              ))}
            </CardContent>
          </Card>
