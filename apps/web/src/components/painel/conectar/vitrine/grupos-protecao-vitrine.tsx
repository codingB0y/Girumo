"use client";

import { useCallback, useEffect, useState } from "react";
import Link from "next/link";
import { ShieldAlert, ShieldCheck, Smartphone, UserPlus, Users } from "lucide-react";

import type { ProtectionSummary } from "@/lib/groups/admin-protection";
import { cenaDaProtecao, textoDeGruposSeus } from "@/lib/painel/saude-numero";

/**
 * Proteção dos grupos na Vitrine Aberta — o único prejuízo irreversível do
 * produto, dito em voz alta.
 *
 * Se o número do lojista cair (ban, troca de aparelho, os 14 dias de linked
 * device) e ele for o único administrador de um grupo, esse grupo fica órfão:
 * ninguém consegue mais administrá-lo, e a lista de clientes não se recupera.
 * O WhatsApp não tem endpoint que devolva a administração de um grupo sem admin.
 *
 * Nenhum número aqui é estimado: vem da contagem gravada em `groups`, apurada
 * no sync e mantida pelo webhook `group-participants.update`.
 */
export function GruposProtecaoVitrine() {
  const [resumo, setResumo] = useState<ProtectionSummary | null>(null);
  const [falhou, setFalhou] = useState(false);

  const carregar = useCallback(async () => {
    try {
      const res = await fetch("/api/groups/protecao");
      if (!res.ok) throw new Error(String(res.status));
      setResumo((await res.json()) as ProtectionSummary);
      setFalhou(false);
    } catch {
      // Painel informativo: uma falha de leitura não pode virar alarme.
      setFalhou(true);
    }
  }, []);

  useEffect(() => {
    void carregar();
  }, [carregar]);

  if (falhou || resumo === null) return null;
  const cena = cenaDaProtecao(resumo);
  if (cena === "nao-se-aplica") return null;

  return (
    <section aria-labelledby="protecao-titulo" className="mt-10">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <h2 id="protecao-titulo" className="font-brand text-20 font-bold text-volt-950">
          Proteção dos seus grupos
        </h2>
        <span className="font-data text-12 uppercase tracking-[0.06em] text-slate-600">
          {textoDeGruposSeus(resumo.administrados)}
        </span>
      </div>
      <p className="mt-1 text-15 text-slate-600">
        Quem mais administra os seus grupos, se o seu número sair do ar.
      </p>

      <div className="mt-4">
        {cena === "risco" && <EmRisco resumo={resumo} />}
        {cena === "protegido" && <Protegido resumo={resumo} />}
        {cena === "nao-medido" && <NaoMedido />}
      </div>
    </section>
  );
}

function EmRisco({ resumo }: { resumo: ProtectionSummary }) {
  const plural = resumo.semBackup !== 1;

  return (
    <article className="pn-card rounded-[var(--radius-control)] p-5 sm:p-6">
      {/* O alerta é o fundo de aviso, não uma borda de destaque — mesmo
          tratamento do aviso de silêncio em numero-saude-vitrine.tsx. */}
      <header className="pn-aviso flex items-start gap-3 rounded-[var(--radius-control)] px-4 py-3">
        <ShieldAlert className="mt-0.5 h-5 w-5 shrink-0 text-danger-700" aria-hidden="true" />
        <div>
          <h3 className="font-brand text-20 font-bold text-volt-950">
            {resumo.semBackup} {plural ? "grupos dependem" : "grupo depende"} só do seu número
          </h3>
          <p className="mt-1 text-13">
            Você é o único administrador {plural ? "deles" : "dele"}. Se esse número for bloqueado ou
            você trocar de aparelho,{" "}
            <strong className="font-semibold">
              {resumo.membrosEmRisco.toLocaleString("pt-BR")}{" "}
              {resumo.membrosEmRisco === 1 ? "pessoa fica" : "pessoas ficam"} num grupo que ninguém
              mais consegue administrar
            </strong>{" "}
            — e não existe como recuperar a lista depois.
          </p>
        </div>
      </header>

      <ul className="mt-4 border-y border-line-200">
        {resumo.emRisco.map((g) => (
          <li
            key={g.id}
            className="flex items-center justify-between gap-4 border-b border-line-200 py-2.5 last:border-b-0"
          >
            {/* Nome de grupo é conteúdo de terceiros: só via escape do JSX. */}
            <span className="truncate text-15 text-volt-950">{g.name}</span>
            <span className="font-data inline-flex shrink-0 items-center gap-1.5 text-12 tabular-nums text-slate-600">
              <Users className="h-3.5 w-3.5" aria-hidden="true" />
              {g.members.toLocaleString("pt-BR")}
            </span>
          </li>
        ))}
      </ul>
      {resumo.semBackup > resumo.emRisco.length && (
        <p className="mt-2 text-12 text-slate-600">
          e mais {resumo.semBackup - resumo.emRisco.length} —{" "}
          <Link href="/painel/grupos" className="text-cobalt-500 underline underline-offset-2">
            ver todos os grupos
          </Link>
        </p>
      )}

      <h4
        id="protecao-saidas"
        className="font-data mt-5 text-12 uppercase tracking-[0.06em] text-slate-600"
      >
        Como resolver
      </h4>
      {/* Lista ORDENADA de propósito: a ordem é a recomendação. A saída de
          graça resolve o problema inteiro, então vem primeiro — inverter isso
          transforma um aviso de risco em anúncio. Em `sm` os dois cartões ficam
          lado a lado, e aí só o DOM carrega essa ordem. */}
      <ol aria-labelledby="protecao-saidas" className="mt-2 grid list-none gap-3 sm:grid-cols-2">
        <Saida
          icone={<UserPlus className="h-4 w-4 text-cobalt-500" aria-hidden="true" />}
          titulo="Promova alguém de confiança"
          preco="de graça, agora"
          descricao="No WhatsApp, abra o grupo, toque no nome do seu sócio ou da sua vendedora e escolha Tornar administrador do grupo. Pronto: se o seu número cair, o grupo continua de pé."
        />
        <Saida
          icone={<Smartphone className="h-4 w-4 text-cobalt-500" aria-hidden="true" />}
          titulo="Conecte um segundo número"
          preco="add-on da assinatura"
          descricao="Um segundo chip seu ligado ao Girumo, que você também torna admin dos grupos. Vira o seu backup sem depender de outra pessoa."
          acao={{ href: "/painel/configuracoes", label: "Ver planos" }}
        />
      </ol>
    </article>
  );
}

function Protegido({ resumo }: { resumo: ProtectionSummary }) {
  return (
    <article className="pn-card rounded-[var(--radius-control)] p-5 sm:p-6">
      <div className="flex items-start gap-3">
        <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-success-700" aria-hidden="true" />
        <div>
          <h3 className="font-brand text-20 font-bold text-volt-950">
            {resumo.comBackup === 1
              ? "Seu grupo tem mais de um administrador"
              : `Seus ${resumo.comBackup} grupos têm mais de um administrador`}
          </h3>
          <p className="mt-1 text-15 text-slate-600">
            Se o seu número sair do ar, alguém continua podendo administrar. É a diferença entre
            perder um número e perder a lista.
          </p>
          {resumo.naoMedidos > 0 && (
            <p className="mt-2 text-13 text-slate-600">
              {resumo.naoMedidos}{" "}
              {resumo.naoMedidos === 1
                ? "grupo ainda não foi conferido"
                : "grupos ainda não foram conferidos"}
              . Sincronize os grupos para incluir na conta.
            </p>
          )}
        </div>
      </div>
    </article>
  );
}

function NaoMedido() {
  return (
    <article className="pn-card rounded-[var(--radius-control)] p-5 sm:p-6">
      <p className="text-15 text-slate-600">
        Ainda não conferimos quem administra os seus grupos.{" "}
        <Link href="/painel/grupos" className="text-cobalt-500 underline underline-offset-2">
          Sincronize os grupos
        </Link>{" "}
        e mostramos aqui quais deles dependem só do seu número.
      </p>
    </article>
  );
}

function Saida({
  icone,
  titulo,
  preco,
  descricao,
  acao,
}: {
  icone: React.ReactNode;
  titulo: string;
  preco: string;
  descricao: string;
  acao?: { href: string; label: string };
}) {
  return (
    <li className="rounded-[var(--radius-control)] bg-canvas-100 p-4">
      <span className="flex items-center gap-2">
        {icone}
        <span className="text-15 font-semibold text-volt-950">{titulo}</span>
      </span>
      <span className="font-data mt-0.5 block text-12 uppercase tracking-[0.06em] text-slate-600">
        {preco}
      </span>
      <p className="mt-2 text-13 text-slate-600">{descricao}</p>
      {acao && (
        <Link
          href={acao.href}
          className="mt-3 inline-flex min-h-11 items-center text-13 font-semibold text-cobalt-500 underline underline-offset-2"
        >
          {acao.label}
        </Link>
      )}
    </li>
  );
}
