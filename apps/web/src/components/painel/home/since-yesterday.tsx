...
export function SinceYesterday({ leadsToday, deltaLeads }: { leadsToday: number; deltaLeads: number }) {
  ...
  // Hoje e ontem zerados. Sumir com o card aqui esconderia justamente o dia em
  // que o lojista precisa saber que nada entrou — silêncio lê como "ainda
  // carregando", não como "parado".
  const isQuiet = leadsToday === 0 && deltaLeads === 0;
      ...
      <div className="mt-2 flex flex-wrap items-center gap-4">
        ...
        {isQuiet && (
          <span className="inline-flex items-center gap-1 rounded-full bg-poco px-2.5 py-1 font-data text-xs text-aco/60">
            ...
            Ontem também não teve
          </span>
        )}
      </div>

      {isQuiet && (
        <p className="mt-3 text-xs text-aco/65">
          Divulgue o link de uma campanha pra voltar a captar — quem clica entra direto no grupo.
        </p>
      )}
    </div>
  );
