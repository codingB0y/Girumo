...
const STATUS_BADGE: Record<string, { label: string; color: string; icon: React.ComponentType<{ className?: string }> }> = {
  scheduled: { label: "Agendado", color: "bg-canvas-100 text-cobalt-500", icon: Clock },
  queued: { label: "Na fila", color: "bg-canvas-100 text-atencao", icon: Loader2 },
  running: { label: "Enviando", color: "bg-canvas-100 text-atencao", icon: Loader2 },
  sent: { label: "Enviado", color: "bg-canvas-100 text-sucesso", icon: CheckCircle2 },
  failed: { label: "Falhou", color: "bg-canvas-100 text-alerta", icon: XCircle },
  draft: { label: "Rascunho", color: "bg-canvas-100 text-slate-600", icon: Send },
};
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={cn(
            ...
            view === "timeline" ? "bg-canvas-100 text-cobalt-500" : "text-slate-600 hover:text-volt-950",
          )}
        >
          Timeline
        </button>
        <button
          ...
          className={cn(
            "rounded-lg px-3 py-1.5 text-xs font-medium transition",
            view === "calendar" ? "bg-canvas-100 text-cobalt-500" : "text-slate-600 hover:text-volt-950",
          )}
        >
        ...
        <div className="rounded-xl border border-line-200 bg-white p-4">
          ...
          <div className="flex items-center justify-between">
            <button
              ...
              className="text-slate-600 hover:text-volt-950"
          ...
          <div className="mt-3 grid grid-cols-7 text-center">
            {["D", "S", "T", "Q", "Q", "S", "S"].map((d, i) => (
              <span key={i} className="font-data text-12 text-slate-600">{d}</span>
            ))}
          </div>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isToday && "bg-canvas-100 font-bold text-cobalt-500",
      ...
      {messages.length === 0 ? (
        ...
        <div className="space-y-5">
          {Object.entries(grouped).map(([day, msgs]) => (
            <div key={day}>
              <h4 className="font-data text-12 mb-2 uppercase tracking-wider text-slate-600">
                {formatDay(day)}
              </h4>
    ...
    <div className="flex items-start gap-3 rounded-xl border border-line-200 bg-white p-3 transition hover:border-line-200">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2">
          <span className={cn("text-12 inline-flex items-center gap-1 rounded-full px-2 py-0.5 font-medium", badge.color)}>
          ...
          {msg.mentionAll && (
            <span className="text-12 inline-flex items-center gap-0.5 text-cobalt-500">
              <AtSign className="h-3 w-3" /> Todos
            </span>
          )}
          {msg.status === "sent" && (
            <span className="font-data text-12 text-sucesso">
              {msg.sent}/{msg.total} grupos
            </span>
          )}
          {msg.error && (
            <span className="text-12 text-alerta">{msg.error}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-1">
        {isScheduled && (
          <button
            ...
            className="rounded-lg p-1.5 text-slate-600 transition hover:bg-canvas-100 hover:text-atencao"
        ...
        {canDelete && (
          <button
            ...
            className="rounded-lg p-1.5 text-slate-600 transition hover:bg-canvas-100 hover:text-alerta"
            title="Excluir"
