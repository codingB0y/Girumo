...
export function CampaignConfig({ mode, slug }: { mode: "create" | "edit"; slug?: string }) {
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-[760px] space-y-4 px-4 py-10 sm:px-6">
  ...
  const sections = [
    <Card key="cadastro">
      ...
      {mode === "edit" && (createdSlug || slug) && (
        <Field label="Link da campanha" hint="É o link que você divulga — ele enche seus grupos.">
          <div className="flex items-center rounded-xl border border-breu/10 bg-bruma/40 px-3.5 py-2.5">
    ...
    <Card key="grupos">
      <div className="flex items-center justify-between">
        ...
        <span className="font-data text-xs uppercase tracking-wider text-aco/55">{selected.size} selecionados</span>
      </div>
          ...
          {groups.map((g) => {
            ...
            return (
              <button
                key={g.id}
                onClick={() => toggle(g.id)}
                className={cn("flex items-center gap-3 rounded-2xl border p-3 text-left transition", sel ? "border-iris bg-iris/[0.05]" : "border-breu/[0.08] bg-white hover:border-iris/30")}
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="font-data text-[11px] text-aco/55">{g.members?.toLocaleString("pt-BR") ?? 0} membros{!g.inviteUrl ? " · sem convite" : ""}</p>
                </div>
  ...
  return (
    <div className="mx-auto max-w-[760px] px-4 py-6 sm:px-6">
      <div className="flex items-center gap-3">
        <button onClick={() => router.push(backHref)} className="flex h-9 w-9 items-center justify-center rounded-lg border border-breu/10 bg-white text-aco transition hover:text-breu">
          <ArrowLeft className="h-4 w-4" />
        </button>
        <h1 className="font-display text-2xl font-extrabold tracking-[-0.03em]">{mode === "edit" ? "Editar campanha" : "Nova campanha"}</h1>
      </div>
      ...
      {mode === "create" ? (
          ...
          {SECTIONS.map((s, i) => (
            <li key={s} className="flex flex-1 items-center last:flex-none">
              <div className="flex items-center gap-2.5">
                <span className={cn("flex h-8 w-8 items-center justify-center rounded-full font-data text-sm font-medium transition", i < idx ? "bg-sucesso text-white" : i === idx ? "bg-iris text-white shadow-iris" : "bg-bruma text-aco/50")}>
        ...
        <div className="mt-6 flex gap-1 border-b border-breu/[0.08]">
          {SECTIONS.map((s, i) => (
            <button key={s} onClick={() => setIdx(i)} className={cn("relative px-4 py-2.5 text-sm font-medium transition", i === idx ? "text-breu" : "text-aco/55 hover:text-breu")}>
              {s}
      ...
      <div className="mt-6 flex items-center justify-between border-t border-breu/[0.08] pt-5">
        ...
        {mode === "create" && idx < SECTIONS.length - 1 ? (
          <button
            ...
            disabled={!canAdvance}
            className={cn("inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-medium text-white transition", canAdvance ? "bg-iris shadow-iris hover:-translate-y-0.5 hover:bg-iris-claro" : "cursor-not-allowed bg-iris/40")}
          >
          ...
          <button
            ...
            className={cn("inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-medium text-white transition", saving || !canAdvance ? "cursor-not-allowed bg-iris/40" : "bg-iris shadow-iris hover:-translate-y-0.5 hover:bg-iris-claro")}
          >
...
function ToggleInline({ on, setOn, labelOn, labelOff }: { on: boolean; setOn: (v: boolean) => void; labelOn: string; labelOff: string }) {
  return (
    <button onClick={() => setOn(!on)} className="flex w-full items-center justify-between rounded-xl border border-breu/10 bg-bruma/40 px-3.5 py-2.5 text-left text-sm transition hover:border-iris/30">
