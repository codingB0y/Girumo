...
export function Lp2Fx() {
  useEffect(() => {
    ...
    if (panelFit && panelScale) {
      // No mobile o CSS muda o aspect-ratio pra 770/600 e aqui cropamos a
      // sidebar (190px) — a área útil fica ~25% maior e legível.
      const cropQuery = window.matchMedia("(max-width: 640px)");
      const applyPanelScale = () => {
        const cropped = cropQuery.matches;
        const canvasWidth = cropped ? 770 : 960;
        const scale = panelFit.clientWidth / canvasWidth;
        const shift = cropped ? -190 * scale : 0;
        panelScale.style.transform = `translateX(${shift}px) scale(${scale})`;
      };
      applyPanelScale();
    ...
    const ctx = gsap.context(() => {
      ...
      // ---- float ambiente do painel do hero (respiração contínua) ----
      gsap.to("[data-lp4-float]", {
        y: -8,
        duration: 3.8,
        ease: "sine.inOut",
        yoyo: true,
