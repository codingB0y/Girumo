  ...
  useEffect(() => {
    fetch("/api/pages/templates")
      ...
      .then((data: LpTemplate[]) => {
        // Por slug, não por posição: `data[0]` é ordem de criação, então um seed
        // novo mudaria em silêncio o modelo de toda página criada daí em diante.
        const v2 = data.find((t) => t.slug === V2_TEMPLATE_SLUG);
        setTemplateId((v2 ?? data[0])?.id ?? null);
      })
